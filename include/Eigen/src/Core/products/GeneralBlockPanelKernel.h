...
void evaluateProductBlockingSizesHeuristic(Index& k, Index& m, Index& n, Index num_threads = 1)
{
  ...
  if (num_threads > 1) {
    ...
    else if(old_k==k)
    {
      ...
      // TODO: part of this blocking strategy is now implemented within the kernel itself, so the L1-based euclidian here should be obsolete.
...
/** \brief Computes the blocking parameters for a m x k times k x n matrix product
  ...
  *   - either by a euclidian based on cache sizes;
