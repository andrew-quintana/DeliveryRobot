...
namespace boost {
  ...
  private:
    ...
    // A "property map" that map which vertices have been deleted
  ...
  run(Vertex s)
  {
    ...
    // be freed as soon as possible so lib it here
    ...
    do {
#ifdef COMPUTE_PATH_COUNTS_INLINE
      // We need to clear the outgoing map after every bucket so just lib it here
