...
// create a pair of a state and a passed id for source and target map
...
// first the source map, transition up to down
// then the target map, up to down
template <class stt>
...
// returns a mpl::vector containing the init map of a state machine
...
// builds a mpl::vector of initial map
...
// returns true if 2 transitions have the same source (used to remove duplicates in search of composite map)
...
// returns true for composite map
...
// transform a transition table in a container of source map
template <class stt>
struct keep_source_names
{
    // instead of the rows we want only the names of the map (from source)
    typedef typename 
        ::boost::mpl::transform<
        stt,transition_source_type< ::boost::mpl::placeholders::_1> >::type type;
};

// transform a transition table in a container of target map
template <class stt>
...
// Template used to create dummy entries for initial map not found in the stt.
...
// used for map created with explicit_creation
...
struct create_stt 
{
    ...
    // iterate through the initial map and add them in the stt if not already there
    typedef typename 
        ::boost::mpl::fold<
        init_states,Stt,
        ::boost::mpl::if_<
                 ...
                 ::boost::mpl::insert< ::boost::mpl::placeholders::_1, ::boost::mpl::end< ::boost::mpl::placeholders::_1>,
                  ...
                  >
        >::type with_init;
    // do the same for map marked as explicitly created
...
// stop iterating on leafs (simple map)
template <class StateType>
...
// returns the transition table for this state + the tables of all composite sub map recursively
template <class Composite>
...
// metafunction used to say if a SM has pseudo exit map
template <class Derived>
...
// builds flags (add internal_flag_list and flag_list). internal_flag_list is used for terminate/interrupt map
template <class StateType>
...
// lib a vector of regions map (as a set)
// one set of map for every region
