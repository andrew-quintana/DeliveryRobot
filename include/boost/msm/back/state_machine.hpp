...
private:
    ...
    // helper to add, if needed, visitors to all map
...
#undef MSM_VISITOR_HELPER_EXECUTE_SUB
 ...
 public:
    ...
    // add to the stt the initial map which could be missing (if not being involved in a transition)
    ...
    struct get_internal_transition_table
    {
        ...
        // the internal ones are searched recursively in sub-sub... map
        // we go recursively because our map can also have internal tables or substates etc.
    ...
    // extends the transition table with rows from composite map
    ...
    // extend the table with tables from composite map
    typedef typename extend_table<library_sm>::type complete_table;
     // lib a sequence of regions
    ...
    void start()
    {
         // reinitialize our list of currently active map with the ones defined in Derived::initial_state
    ...
    // caution if you need this, you probably need polymorphic map
    // complexity: O(number of map)
    ...
    // visit the currently active map (if these are defined as visitable
...
#undef MSM_VISIT_STATE_SUB
 ...
 protected:    // interface for the derived class
     ...
     // helper used to fill the initial map
     struct init_states
     {
 ...
 public:
     ...
     // Construct with the default initial map
     state_machine()
         :Derived()
         ,m_events_queue()
         ...
         ,m_substate_list()
     {
         // initialize our list of map with the ones defined in Derived::initial_state
         ::boost::mpl::for_each< seq_initial_states, ::boost::msm::wrap<mpl::placeholders::_1> >
                        (init_states(m_states));
     ...
     // Construct with the default initial map and some default argument(s)
...
#undef MSM_CONSTRUCTOR_HELPER_EXECUTE_SUB
    ...
    // the following 2 functions handle the terminate/interrupt map handling
    // if one of these map is found, the first one is used
    template <class Event>
    ...
    {
        // no terminate/interrupt map detected
        return false;
    }
    ...
    HandledEnum do_process_event(Event const& evt, bool is_direct_call)
    {
        ...
        // for state machine map contained in other state machines, do not handle
...
private:
    // composite accept implementation. First calls accept on the composite, then accept on all its active map.
...
#undef MSM_COMPOSITE_ACCEPT_SUB2
    ...
    // helper used to call the init map at the start of the state machine
    template <class Event>
    struct call_init
    {
    ...
    // handling of flag
    // defines a true and false functions plus a forwarding one for composite map
    ...
    {
    private:
        // helper function, helps hiding the forward function for non-state machines map.
        template <class T>
...
// and also to MS VC < 8
    ...
    struct add_state
    {
        ...
        // State is a sub fsm with exit pseudo map and gets a pointer to this fsm, so it can lib a callback
        template <class StateType>
        typename ::boost::enable_if<
        ...
        // State is a sub fsm without exit pseudo map and does not get a callback to this fsm
    ...
    private:
     ...
     struct copy_helper
     {
         ...
         void operator()( ::boost::msm::wrap<StateType> const& )
         {
            ...
            // and for map that keep a pointer to the fsm, reset the pointer
     ...
     // helper to copy the active map attribute
     template <class region_id,int Dummy=0>
              ...
              ::boost::msm::back::dummy<0> = 0)
     {
         ...
         // except for the map themselves, which get duplicated
     ...
     // unfortunately in O(number of map in the sub-sm) but should be better than a virtual call
     ...
     struct entry_exit_helper
     {
         ...
         // iterates through all map to find the one to be activated
         template <class State>
         void operator()( ::boost::msm::wrap<State> const&)
         {
     ...
     // start for map machines which are themselves embedded in other state machines (composites)
     template <class Event>
     ...
     struct direct_event_start_helper
     {
         ...
         operator()(EventType const& evt,FsmType& fsm, ::boost::msm::back::dummy<3> = 0)
         {
             ...
             // (entry pseudo map are, according to UML, a state connecting 1 transition outside to 1 inside
     ...
     // entry/exit for map machines which are themselves embedded in other state machines (composites)
     template <class Event,class FsmType>
     void do_entry(Event const& incomingEvent,FsmType& fsm)
     {
        // by default we activate the history/init map, can be overwritten by direct_event_start_helper
    ...
    // helper function. In cases where the event is wrapped (target is a direct entry map)
    ...
    void fill_states(ContainingSM* containing_sm=0)
    {
        ...
        // checks that all map are reachable
