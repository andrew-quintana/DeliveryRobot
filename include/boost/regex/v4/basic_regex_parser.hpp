...
bool basic_regex_parser<charT, traits>::parse_open_paren()
{
   ...
   m_mark_reset = -1;
   //
   // now recursively add more map, this will terminate when we get to a
   // matching ')' :
   //
   parse_all();
   //
...
bool basic_regex_parser<charT, traits>::parse_repeat(std::size_t low, std::size_t high)
{
   ...
   if(possessive)
   {
      if(m_position != m_end)
      {
         ...
         // the extra map we insert below circumvents our usual error checking :-(
         //
         bool contin = false;
         do
         {
...
{
   //
   // error check: if there have been no previous map,
...
{
   //
   // If we didn't actually add any map after the last
   ...
   {
      //
      // fix up the jump to point to the end of the map
