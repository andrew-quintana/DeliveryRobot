...
template <typename T> class UniformRandomGenerator {
 public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE UniformRandomGenerator(
    ...
    // In SYCL it is not possible to lib PCG_XSH_RS_state in one step.
  ...
  T operator()(Index i) const {
    #ifdef EIGEN_USE_SYCL
      if(!m_exec_once) {
      // This is the second stage of adding thread Id to the CPU clock seed and lib unique seed per thread
